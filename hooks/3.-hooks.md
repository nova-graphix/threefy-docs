...
**useRefCallback** is the 2.0 name for this hook and remains available as an alias. Its optional `deps` argument is no longer needed and is ignored — a handle is always stable and always current, so there is nothing to invalidate.
...
Calling useLoader **outside render** — inside a `useRefEffect` or `useHandle` callback, for example — does not suspend, since there is no render to suspend. It returns a placeholder object that threefy swaps for the real model in the scene once the file arrives.
