...
> **Note:** `<Model>` and `<InstancedModel>` are **not yet exported by the `threefy` package** in the current v2 release — in the examples they run as demo-app helpers. Support is **planned for an upcoming release**. Until then, load and place models with the exported `useLoader` hook and `<primitive>` element, and instance whole objects with `<instancedObject>`.
