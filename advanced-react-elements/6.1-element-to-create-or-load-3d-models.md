...
> **Note:** `<Model>` and `<InstancedModel>` are **not part of the `threefy` package**. They are components the demo app builds on top of the exported API, described here because the pattern is a useful one — you can see them at work in [7.8 Demo source code](../examples/7.8-demo-sources.md). To load and place a model with what the package does export, use the `useLoader` hook with `<primitive>`, and `<instancedObject>` to instance a whole object.
